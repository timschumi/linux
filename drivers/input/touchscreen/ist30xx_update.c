...
#include "./Imagis/ist30xxb_fw_kyle-ve.h"
...
int ist30xxb_cmd_read_chksum(struct i2c_client *client, u32 *chksum)
{
	int ret = 0;
	u32 val = (1 << 31); // Chkecksum enable

	val |= (ts_tags->fw_addr);
	val |= (ts_tags->flag_addr / IST30XX_DATA_LEN - 1) << 16;

	ret = ist30xx_write_cmd(client, IST30XXB_REG_CHKSMOD, val);
	if (ret) {
		tsp_err("ISP fail, IST30XXB_REG_CHKSMOD\n");
		return ret;
	}
	...
	if (ret) {
		tsp_err("ISP fail, IST30XXB_REG_CHKSDAT\n");
		return ret;
	}

	tsp_debug("Chksum: %x (%x~%x)\n",
		  *chksum, ts_tags->fw_addr, ts_tags->flag_addr);
...
int ist30xxb_isp_fw_update(struct i2c_client *client, const u8 *buf, u32 *chksum)
{
	...
	ret = ist30xxb_cmd_read_chksum(client, chksum);
	if (ret) goto isp_fw_update_end;
...
static DEVICE_ATTR(fwparam, 0666, NULL, ist30xx_fwparam_store);
static DEVICE_ATTR(boot, 0666, ist30xx_fw_show, ist30xx_boot_store);
